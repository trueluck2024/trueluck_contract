/**
 *Submitted for verification at polygonscan.com on 2025-07-31
 */
    ...
    ReentrancyGuard
{
    ...
    uint256 public subscriptionId =
        24984571401401149363890098904988541861156174354728957142577061483844090176362;
    ...
    function updateAdminWallet(address _newAdminWallet) external onlyMyOwner {
    ...
    function createRoom(
        ...
        rooms.push(
            Room({
                ...
                PolygonExclusive: _PolygonExclusive,
                refundDelete: false,
        ...
        )
    {
        ...
        PolygonExclusive = new bool[](count);
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            PolygonExclusive[i] = room.PolygonExclusive;
    ...
    function RoomDeleteAndRefund(uint256 roomId)
        external
        onlyOwner
        nonReentrant
    {
        ...
        // Deactivate room
        room.active = false;
        room.refundDelete = true; // if user Refund the amount to use to retrieve
    ...
    // Buy Tickets
    function buyTickets(uint256 roomId, uint256 usdtAmount)
        public
        nonReentrant
    {
        require(roomId > 0 && roomId <= rooms.length, "Invalid roomId");
    ...
    event AmountMessage(string amount);
    event lzcomposeMessage(address from);
    ...
    function lzCompose(
        ...
        emit lzcomposeMessage(_from);
        try this._tryProcessTickets(_message) {
            emit AmountMessage("success");
        } catch Error(string memory reason) {
            emit AmountMessage(reason);
            this._fallbackRefund(_message);
        } catch {
            this._fallbackRefund(_message);
            emit AmountMessage("failure");
        }
    }

    event amountTransfer(address user, uint256 amount);

    function _fallbackRefund(bytes calldata _message) external {
        require(msg.sender == address(this), "Only internal call allowed");
        uint256 amountLD = OFTComposeMsgCodec.amountLD(_message);
        bytes memory _composeMessage = OFTComposeMsgCodec.composeMsg(_message);
        (address sender, uint256 roomId, uint256 _amount) = abi.decode(
            _composeMessage,
            (address, uint256, uint256)
        );
        usdtToken.safeTransfer(sender, _amount);
        emit amountTransfer(sender, amountLD);
    }

    function _tryProcessTickets(bytes calldata _message) external {
        require(msg.sender == address(this), "Only internal call allowed");
        _processTickets(_message);
    }

    function _processTickets(bytes calldata _message) internal {
        bytes memory _composeMessage = OFTComposeMsgCodec.composeMsg(_message);
        (address sender, uint256 roomId, uint256 _amount) = abi.decode(
            _composeMessage,
            (address, uint256, uint256)
        ...
        emit TicketsBought(sender, roomId, ticketsToBuy);
    }
        ...
        if (room.autoOpen) {
            ...
            rooms.push(
                Room({
                    ...
                    autoOpen: false,
                    winnerSelected: false,
                    PolygonExclusive: room.PolygonExclusive,
                    refundDelete: false,
                    createdAt: block.timestamp
                })
            );
